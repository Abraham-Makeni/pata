...
const mockBookings = [
  { id: '1', provider: 'Kevo Cuts', service: 'Fade + Line-up', date: 'May 2, 10:30 AM', status: 'Upcoming' },
  { id: '4', provider: 'Nailhaus', service: 'Gel Manicure', date: 'May 4, 2:00 PM', status: 'Upcoming' },
]

const followedBusinessIds = ['1', '4', '7']

export default function ProfilePage({ params }: { params: { id: string } }) {
  const router = useRouter()
  const [selectedPost, setSelectedPost] = useState<string | null>(null)
  const [isFollowing, setIsFollowing] = useState(false)
  const [uploadedBusinessImages, setUploadedBusinessImages] = useState<string[]>([])
  const p = getProviderById(params.id)

  useEffect(() => {
    if (params.id === 'me' || !p) {
      setUploadedBusinessImages([])
      return
    }

    const storageKey = `businessUploads:${p.id}`
    const storedUploads = window.localStorage.getItem(storageKey)
    if (!storedUploads) {
      setUploadedBusinessImages([])
      return
    }
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        setUploadedBusinessImages(parsed)
      } else {
        setUploadedBusinessImages([])
      }
    } catch {
  ...
  const profileGalleryImages = [...uploadedBusinessImages, ...p.galleryImages]

  const getProviderDistance = () => {
    if (!p.coordinates) return 'Distance unavailable'
    const nairobiCenter = { lat: -1.2921, lng: 36.8219 }
    const latDiff = p.coordinates.lat - nairobiCenter.lat
    const lngDiff = p.coordinates.lng - nairobiCenter.lng
    const distanceKm = Math.sqrt((latDiff ** 2) + (lngDiff ** 2)) * 111
    return `${distanceKm.toFixed(1)} km away`
  }

  const getProviderOpenStatus = () => {
    const isOpen = Number(p.id) % 2 === 1
    return {
  ...
  return (
    <div className="min-h-screen bg-surface-soft">
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10">
          <div className="px-5 pb-4">
            <div className="flex items-center gap-4">
              ...
              <div className="img-placeholder w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-xl flex-shrink-0">
      ...
      {/* ── BODY ── */}
      <div className="px-5 pb-48 pt-12 space-y-6">
        ...
        <Section title="Posts">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide mb-3">
            {profileGalleryImages.slice(0, Math.ceil(profileGalleryImages.length / 2)).map((img, i) => (
              <div 
                key={i} 
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {profileGalleryImages.slice(Math.ceil(profileGalleryImages.length / 2)).map((img, i) => (
              <div 
                key={i + Math.ceil(profileGalleryImages.length / 2)} 
                className="flex-shrink-0 w-32 cursor-pointer group"
                onClick={() => setSelectedPost(img)}
              >
                <div className="img-placeholder aspect-square rounded-xl overflow-hidden">
                  <img 
                    src={img} 
                    alt={`${p.name} work ${i + Math.ceil(profileGalleryImages.length / 2) + 1}`} 
                    className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" 
                    onError={e=>{(e.target as HTMLImageElement).style.display='none'}}
                  />
                </div>
        ...
        >
          ...
          >
            ...
            <div className="p-4">
              ...
              {/* Location */}
              <button
                onClick={() => router.push(`/maps?providerId=${p.id}`)}
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <p className={`text-[12px] font-medium ${getProviderOpenStatus().className}`} style={{ fontFamily: 'var(--font-outfit)' }}>
                    {getProviderOpenStatus().label}
                  </p>
...
function UserAccountPage() {
  ...
  const [showAllFollowedBusinesses, setShowAllFollowedBusinesses] = useState(false)
  const [bookings, setBookings] = useState(mockBookings)
  const [bookingToCancel, setBookingToCancel] = useState<string | null>(null)
  const router = useRouter()
  const followedBusinesses = followedBusinessIds
    .map((id) => getProviderById(id))
    .filter((provider): provider is NonNullable<typeof provider> => Boolean(provider))
  const visibleFollowedBusinesses = showAllFollowedBusinesses
    ? followedBusinesses
    : followedBusinesses.slice(0, FOLLOWED_PREVIEW_LIMIT)
    ...
    <div className="min-h-screen bg-surface-soft pb-8">
      ...
      <div className="px-5 pt-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
          {bookings.length === 0 ? (
            <div className="p-4 text-[13px] text-ink-muted" style={{ fontFamily:'var(--font-outfit)' }}>
              You currently have no active bookings.
            </div>
          ) : (
            bookings.map((booking, index) => (
              <div key={booking.id} className={`p-4 ${index !== bookings.length - 1 ? 'border-b border-surface-border' : ''}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-[14px] font-semibold" style={{ fontFamily:'var(--font-outfit)' }}>
                      {booking.provider}
                    </p>
                    <p className="text-[12px] text-ink-secondary mt-0.5" style={{ fontFamily:'var(--font-outfit)' }}>
                      {booking.service}
                    </p>
                    <p className="text-[12px] text-ink-muted mt-1" style={{ fontFamily:'var(--font-outfit)' }}>
                      {booking.date}
                    </p>
                  </div>
                  <span className="text-[10px] px-2 py-1 rounded-full bg-brand/10 text-brand font-semibold" style={{ fontFamily:'var(--font-outfit)' }}>
                    {booking.status}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-3">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-surface-soft text-[12px] font-semibold text-ink-secondary hover:bg-surface-muted transition-all"
                    style={{ fontFamily:'var(--font-outfit)' }}
                  >
                    Edit
                  </button>
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-red-50 text-[12px] font-semibold text-red-600 hover:bg-red-100 transition-all"
                    style={{ fontFamily:'var(--font-outfit)' }}
                  >
                    Cancel
                  </button>
      ...
      <div className="px-5 pt-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
          {visibleFollowedBusinesses.map((business, index) => (
            <button 
              key={business.id}
              onClick={() => router.push(`/profile/${business.id}`)}
              className={`w-full p-4 hover:bg-surface-soft transition-all ${index !== visibleFollowedBusinesses.length - 1 ? 'border-b border-surface-border' : ''}`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-surface-soft overflow-hidden flex-shrink-0">
                    <img src={business.avatarImage} alt={business.name} className="w-full h-full object-cover" onError={e=>{(e.target as HTMLImageElement).style.display='none'}} />
                  </div>
                  <div className="text-left">
                    ...
                    <p className="text-[12px] text-ink-muted font-medium" style={{ fontFamily:'var(--font-outfit)' }}>
                      {business.tags[0]} · {business.location}
                    </p>
                  </div>
                </div>
          ...
          {followedBusinesses.length > FOLLOWED_PREVIEW_LIMIT && (
            <button
              onClick={() => setShowAllFollowedBusinesses((prev) => !prev)}
              className="w-full p-3 border-t border-surface-border text-[13px] font-semibold text-brand hover:bg-brand/5 transition-all"
              style={{ fontFamily:'var(--font-outfit)' }}
            >
              {showAllFollowedBusinesses
                ? 'Show less'
                : `Show ${followedBusinesses.length - FOLLOWED_PREVIEW_LIMIT} more`}
            </button>
          )}
        </div>
      </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[12px] text-ink-muted font-medium" style={{ fontFamily:'var(--font-outfit)' }}>
                    Expand to call support or report an issue
                  </p>
                </div>
              </div>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`text-ink-muted transition-transform ${showSupportMenu ? 'rotate-90' : ''}`}><polyline points="9 18 15 12 9 6"></polyline></svg>
            </div>
          </button>
          ...
          {showSupportMenu && (
            <div className="px-4 pb-4 space-y-3 border-t border-surface-border">
              <button 
                ...
                className="w-full p-3 mt-3 rounded-xl bg-surface-soft hover:bg-surface-muted transition-all text-left"
              >
                <p className="text-[14px] font-semibold" style={{ fontFamily:'var(--font-outfit)' }}>
                  Call Support
                </p>
              ...
              <div className="rounded-xl bg-surface-soft p-3">
                <p className="text-[13px] font-semibold mb-2" style={{ fontFamily:'var(--font-outfit)' }}>
                  Common Issues
                </p>
                <div className="space-y-2">
                  {helpIssues.map((issue, index) => (
                    <button
                      ...
                      className="w-full text-left p-2 rounded-lg bg-white hover:bg-surface-muted transition-all"
                    >
                      <p className="text-[12px] font-medium text-ink-secondary" style={{ fontFamily:'var(--font-outfit)' }}>
                        {issue}
                      </p>
                    </button>
                  ))}
              ...
              <button 
                onClick={handleEmailSupport}
                className="w-full p-3 bg-brand text-white rounded-xl text-center text-[14px] font-semibold hover:bg-brand/90 transition-all flex items-center justify-center gap-2"
                style={{ fontFamily:'var(--font-outfit)' }}
              >
      ...
      {bookingToCancel && (
        ...
        >
          ...
          >
            ...
            <p className="text-[13px] text-ink-muted mb-5" style={{ fontFamily:'var(--font-outfit)' }}>
              This will remove the booking from your upcoming appointments.
            </p>
            <div className="flex items-center gap-3">
              <button
                onClick={() => setBookingToCancel(null)}
                className="flex-1 py-2.5 rounded-xl border border-surface-border text-[13px] font-semibold text-ink-secondary hover:bg-surface-soft transition-all"
                style={{ fontFamily:'var(--font-outfit)' }}
              >
                Keep booking
              </button>
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-red-600 text-white text-[13px] font-semibold hover:bg-red-700 transition-all"
                style={{ fontFamily:'var(--font-outfit)' }}
              >
                Yes, cancel
              </button>
